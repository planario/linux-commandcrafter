...
| 4 | `CommandAnalyzer.tsx` | `process.env.API_KEY` — Vite does not support `process.env` in browser code | Changed to `import.meta.env.VITE_API_KEY` |
| 5 | `CommandAnalyzer.tsx` | Model name `'gemini-3-flash-preview'` does not exist | Changed to `'gemini-2.0-flash'` |
| 6 | `docker-compose.yml` | `API_KEY` passed as runtime env var to nginx, but Vite bakes env vars at **build** time | Changed to build arg `VITE_API_KEY` |
...
export VITE_API_KEY=your_gemini_api_key_here
...
```bash
npm install
VITE_API_KEY=your_key npm run build
